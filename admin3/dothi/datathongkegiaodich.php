...

{
 ...
 SELECT concat(extract(day from ngaymua),'-',extract(month from ngaymua),'-',	extract(year from ngaymua)) as ngaymua,count(*) as soluongdonhang
  FROM donhang
  where extract(year from ngaymua)=".$nam." and extract(month from ngaymua)=".$thang."
  group by extract(day from ngaymua)
  ORDER BY ngaymua ASC";
	}
	else{
$query = "
 SELECT ngaymua as ngaymua,count(*) as soluongdonhang
  FROM donhang
  where extract(year from ngaymua)=".$nam." 
